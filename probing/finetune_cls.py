"""Full fine-tuning of frozen Chronos-2 on a CLASSIFICATION task (FordA) — TASK-SHIFT, Exp-A source.
...
the SAME frozen Chronos-2 on a classification task and check whether changing the TASK creates
stronger late-layer specialization than changing only the forecasting domain. It writes two
checkpoints of ONE run so run_task_shift can probe stage0 (pretrained) / stage1 (early) / stage2
(late) with the same layerwise-linear lens.

Head + forward (the ONE fixed pooling rule):
    enc = model.encode(context=ctx, num_output_patches=1)[0]   # (b, ncp+1+1, 768) POST final-LN
    pooled = enc[:, :ncp, :].mean(1)                           # mean over ncp=ceil(500/16)=32 content tokens
    logits = Linear(768, 2)(pooled) ; loss = CrossEntropy(logits, y)
``model.encode`` is fully differentiable (extraction wraps it in no_grad EXTERNALLY), so gradients
flow into the whole ~119M backbone. K=1 here (num_output_patches=1) is identical to the K used when
run_task_shift extracts the classification features — attention is non-causal, so K MUST match FT<->probe.
The pooled state read is the POST-final-LN state, i.e. exactly the L12+LN (index 13) probe point.
...
Optimizer: AdamW, TWO param groups (backbone LR 1e-5, head LR 1e-3), linear decay, warmup 5%,
grad-clip 1.0, batch 64, seed 0.
...
CHECKPOINT RULE (user, 2026-08-18 — deterministic, NOT tuned on any probe curve):
    ...
    stage2_cls_late  = the BEST-VAL-ACCURACY checkpoint STRICTLY AFTER stage1, provided backbone
                       ...
                       a late stage). Deciding uses val-acc + backbone drift only.

Runnable (the C1 job calls this):
    python -m probing.finetune_cls                # 10 epochs, batch 64, backbone 1e-5 / head 1e-3, seed 0
...
SOURCE = "forda_cls"                 # cache/checkpoint source label (disjoint from BOOM's "boom")
TAG = "forda"                        # HF/aeon tag inside the source
...
# --------------------------------------------------------------------------- #
def build_cls_head(d_model: int, n_classes: int, device=None) -> nn.Linear:
    """The classification head is a SINGLE Linear(d_model, n_classes) — strictly linear, no hidden
    layer, no activation. (Tested: isinstance nn.Linear and forward == W x + b.)"""
...
def pool_content_cls(hs: torch.Tensor, ncp: int) -> torch.Tensor:
    """Mean-pool the first ``ncp`` content tokens of a (b, P, d) hidden state -> (b, d). The ONE
    fixed pooling rule shared by the FT forward and the probe-feature extractor."""
...
# --------------------------------------------------------------------------- #
# evaluation on the fixed FordA validation split (dropout OFF)
# --------------------------------------------------------------------------- #
@torch.no_grad()
def eval_cls_val(model, head, X, y, ncp, batch_size, device, autocast_ctx) -> tuple[float, float]:
    """(val_accuracy, val_cross_entropy) over the fixed FordA validation split, in eval mode."""
    ...
    try:
        for b0 in range(0, len(X), batch_size):
            ...
            with autocast_ctx():
                enc_out, *_ = model.encode(context=ctx, num_output_patches=1)
                logits = head(pool_content_cls(enc_out[0], ncp).float())
...
# --------------------------------------------------------------------------- #
def finetune_cls(*, epochs: int = 10, batch_size: int = 64, backbone_lr: float = 1e-5,
                 head_lr: float = 1e-3, seed: int = 0, warmup_ratio: float = 0.05,
                 max_grad_norm: float = 1.0, ckpt_root=None, out_root=None, device=None,
                 model_id: str = MODEL_ID) -> dict:
    """Full classification fine-tuning of Chronos-2 on FordA; write stage1_cls_early (end of epoch 1)
    and, if the validity gate passes, stage2_cls_late (best-val after stage1 with increased backbone
    drift). Returns the manifest dict. Checkpoints -> ckpt_root/forda_cls/<stage>/; manifest +
    histories -> out_root/forda_cls/."""
    spec = CLS_SPECS[TAG]
    n_classes = spec["n_classes"]
    ncp = math.ceil(spec["length"] / OUTPUT_PATCH_SIZE)
    ...
    # --- data (RAW contexts; model normalizes internally) ---
    data = load_forda(TAG)
    ...
    print(f"[ft-cls] FordA: {n_train} train / {len(Xva)} val | ncp={ncp} n_classes={n_classes} "
          f"batch={batch_size} epochs={epochs} steps/epoch={steps_per_epoch} num_steps={num_steps} "
          f"warmup={num_warmup} backbone_lr={backbone_lr} head_lr={head_lr}")
    ...
    head = build_cls_head(model.config.d_model, n_classes, device=device)
    ...
    def save_stage(stage_label):
        ckpt_dir = ckpt_root / SOURCE / stage_label
        ...
        torch.save({"state_dict": {k: v.cpu() for k, v in head.state_dict().items()},
                    "in_features": model.config.d_model, "n_classes": n_classes},
    ...
    v0_acc, v0_ce = eval_cls_val(model, head, Xva, yva, ncp, batch_size, device, autocast_ctx)
    ...
    for epoch in range(1, epochs + 1):
        ...
        va_acc, va_ce = eval_cls_val(model, head, Xva, yva, ncp, batch_size, device, autocast_ctx)
        ...
        if epoch == 1:                                    # stage1 = end of epoch 1 (deterministic)
            ...
            stage1_info = {"step": step, "epoch": 1, "stage": STAGE1, "checkpoint_hash": h,
                           "checkpoint_dir": cdir, "cache_prefix": ft_cache_prefix(TAG, SOURCE, STAGE1, h),
            ...
            if best_late is None or va_acc > best_late["val_acc"]:
                ...
                best_late = {"step": step, "epoch": epoch, "stage": STAGE2, "checkpoint_hash": h,
                             "checkpoint_dir": cdir,
                             "cache_prefix": ft_cache_prefix(TAG, SOURCE, STAGE2, h),
                             "val_acc": va_acc, "val_ce": va_ce, "backbone_drift_l2": d_scalar,
                             "param_drift": drift}
    ...
    if late_found:
        ...
        # mislabeled leftover
        stale = ckpt_root / SOURCE / STAGE2
        if stale.exists():
            shutil.rmtree(stale)

    manifest = {
        "experiment": "task_shift_classification_ft", "source": SOURCE, "tag": TAG,
        "aeon_name": spec["aeon_name"], "model_id": model_id, "finetune_mode": "full",
        "trainable_params": int(n_trainable), "n_classes": n_classes, "ncp": ncp,
        "head": "Linear(d_model, n_classes)  # strictly linear, no hidden layer",
        ...
        "geometry": {"length": spec["length"], "ncp": ncp, "num_output_patches": 1,
                     "P": OUTPUT_PATCH_SIZE},
...
def _parse_args(argv=None):
    ap = argparse.ArgumentParser(description="Classification full fine-tuning of Chronos-2 (FordA).")
...
def main(argv=None):
    a = _parse_args(argv)
    finetune_cls(epochs=a.epochs, batch_size=a.batch_size, backbone_lr=a.backbone_lr,
                 head_lr=a.head_lr, warmup_ratio=a.warmup_ratio, seed=a.seed, device=a.device)
