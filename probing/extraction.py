...
# ----------------------------------------------------------------------- #
# Windowed ID-forecasting extraction (Phase 0; additive, does not touch the
# UEA classification path above). Memory-safe: pools each batch immediately
# instead of accumulating all (n, P, 768) hidden states, so a large window
# count stays well within 24 GB on MPS. Cached under an ``IDF_`` prefix so it
# can never collide with the UEA classification cache.
# ----------------------------------------------------------------------- #
...
def extract_window_features(tag, split, contexts, y, pooling="content", batch_size=128):
    ...
    pooling   : "content" | "all" | "reg" (same layout as the UEA path).
