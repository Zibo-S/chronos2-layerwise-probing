...
def latency_by_depth(lat, batch):
    """Timings for one batch size, referenced to the separately measured NATIVE model.

    Returns {"by_depth", "reference", "denominator", "native_measured"}. The reference matters: a
    depth-12 truncation still carries the linear adapter spliced in front of the final RMSNorm, so
    it is NOT the pretrained model. Runs that measured a native baseline (condition == "native")
    use it; older runs fall back to the depth-12 adapter model and say so, so the distinction is
    never silently glossed.
    """
    rows = {r["depth"]: r for r in lat["rows"]
            if r["batch_size"] == batch and r.get("condition", "truncated") != "native"}
    if not rows:
    ...
    native = next((r for r in lat["rows"]
                   if r["batch_size"] == batch and r.get("condition") == "native"), None)
    measured = native is not None
    ref = native if measured else rows.get(model_size.NUM_BLOCKS)
    if ref is None:
        raise ValueError("the latency run measured neither a native baseline nor the full "
                         "12-block model, so no speedup can be referenced to anything")
    for r in rows.values():
        r["speedup"] = ref["predict_median_ms"] / r["predict_median_ms"]
        r["memory_ratio"] = (r["peak_mib"] / ref["peak_mib"]) if "peak_mib" in r else None
    denom = ("the separately measured native model" if measured else
             "the depth-12 truncation, which still carries the linear adapter; this run measured "
             "no separate native baseline")
    return {"by_depth": rows, "reference": ref, "denominator": denom, "native_measured": measured}
...
def latex_full_table(rows_by_rule, tags, lat, batch):
    ...
    L = latency_by_depth(lat, batch)
    lut, ref = L["by_depth"], L["reference"]
    ...
    for i, tag in enumerate(tags):
        ...
        out.append("    " + " & ".join(cells) + r" \\")
    out += [r"    \bottomrule", r"\end{tabular}",
            f"% Speedup = median wall-clock of {L['denominator']} divided by that of the truncated",
            f"% model, batch {batch}, on {gpu} ({ref['predict_median_ms']:.2f} ms reference).",
            f"% Depends on truncation depth only, not on the dataset.",
            f"% generated by: python -m experiments.run_compression_cost --latex-appendix"]
    return "\n".join(out)


def _cudnn(v):
    """Decode torch.backends.cudnn.version(): major*10000 + minor*100 + patch."""
    if v in (None, ""):
        return MISSING
    v = int(v)
    return f"{v // 10000}.{(v % 10000) // 100}.{v % 100} (raw {v})"
...
def latex_appendix_methodology(lat, batch):
    ...
    vout = e.get("verification_outputs") or {}
    out_gate = (f"exactly {float(vout['max_abs']):.1e}" if vout else None)
    LREF = latency_by_depth(lat, batch)
    ...
    L = [r"\section{Latency and Memory Methodology}", r"\label{app:latency-methodology}", "",
         ...
         r"and each truncated variant under an identical protocol, with the model reloaded",
         r"independently for each depth.", "",
         ...
         r"its last remaining block. Before timing, the harness checks two things: that the",
         r"truncated encoder reproduces the hooked full-model states at every measured depth, and",
         r"that at depth 12 with an identity-initialised adapter the spliced path reproduces the",
         r"unmodified model's final quantile forecasts. The observed maximum absolute deviation on",
         (f"the hidden states was {gate}" if gate else
          r"\textbf{[state gate not run --- pass \texttt{--verify}]}"),
         (f", and on the final forecasts {out_gate}, i.e.\\ numerically identical in our "
          r"verification." if out_gate else
          r"\textbf{[output gate not run --- pass \texttt{--verify}]}."), "",
         ...
         f"cuDNN {_cudnn(e.get('cudnn'))}), model \\texttt{{{_tex(e.get('model_id'))}}}. ",
         ...
         f"timed over {ref['reps']} repetitions. CUDA synchronisation is performed before and after",
         r"each timed region to account for asynchronous execution. We report the \emph{median} over",
         r"repetitions, which is robust to occasional scheduling outliers, and the 95th percentile",
         r"separately as the tail. Two timings are recorded because they answer different questions:",
         r"\texttt{predict} calls \texttt{Chronos2Pipeline.predict\_quantiles} on host arrays and",
         r"therefore \emph{includes} preprocessing and the host-to-device transfer, i.e.\ the",
         r"complete pipeline prediction path used in our benchmark; \texttt{encode} times the",
         r"encoder forward alone on a tensor already resident",
         ...
         r"not use \texttt{nvidia-smi}, which reports the caching allocator's reservation rather than",
         r"live allocations. The weight-only figure is the allocated total after loading and",
         r"truncating but before any forward pass. The model is reloaded independently at each depth",
         r"so that parameters from removed blocks do not remain allocated.", "",
         ...
         r"lookups into this table at each dataset's selected depth. Speedups are referenced to "
         + LREF["denominator"] + ", measured under the same protocol"
         + (f" ({LREF['reference']['predict_median_ms']:.2f}\\,ms at batch {batch})." if LREF.get("reference")
            else "."), ""]

    T = [r"\begin{table}[t]", r"    \centering", r"    \small",
         (r"    \caption{Measured latency, throughput and memory. \texttt{Native} is the "
          r"unmodified pretrained model; the \(L\ell\) rows are truncated at $\ell$ blocks and "
          r"carry the linear adapter, so \(L12\) is \emph{not} the native model. "
          r"\texttt{predict} is the complete pipeline prediction path (transfer included); "
          r"\texttt{encode} is the encoder forward on a device-resident tensor (transfer "
          r"excluded). Speedups in the main text are referenced to " + LREF["denominator"] + ".}"),
    ...
    for b in batches:
        ...
        # the native baseline first, then the truncated depths
        sub.sort(key=lambda r: (r.get("condition", "truncated") != "native", r["depth"]))
        for j, r in enumerate(sub):
            lab = (r"\texttt{Native}" if r.get("condition") == "native"
                   else f"\\({r['depth_label']}\\)")
            T.append(f"        {b if j == 0 else ''} & {lab} & {r['depth']} & "
...
def main():
    ...
    if args.latex_appendix:
        ...
        L = latency_by_depth(lat, args.speedup_batch)
        print(f"\n[latency] {lat.get('_source')} on {lat['environment'].get('gpu_name')} "
              f"— speedup at batch {args.speedup_batch} vs {L['denominator']}:")
        if not L["native_measured"]:
            print("  [warn] no native baseline in this run; the reference still carries the "
                  "adapter. Re-run job_latency.sh to measure one.")
        for d in sorted(L["by_depth"]):
            r = L["by_depth"][d]
